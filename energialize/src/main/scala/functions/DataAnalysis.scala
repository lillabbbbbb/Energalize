...
object DataAnalysis {
    ...
    def mean(list: List[Double]): Double = if (list.isEmpty) 0.0 else list.sum / list.size

    def median(list: List[Double]): Double = {
        val listSize = list.size
        if (listSize == 0) 0.0 else {
            val sortedList = list.sorted
            if (listSize % 2 == 0) ((sortedList(listSize / 2)) + (sortedList(listSize / 2 - 1))) / 2
            else sortedList(listSize / 2)
        }
    }
    ...
    def mode(list: List[Double]): Double = { // source used: https://stackoverflow.com/questions/21998521/how-to-find-the-mode-value-of-a-list
        if (list.isEmpty) 0.0 else {
            // count occurrences
            val counts = list.groupBy(identity).map { case (k, v) => k -> v.size }
